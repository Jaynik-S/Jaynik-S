### <img src="https://raw.githubusercontent.com/MartinHeinz/MartinHeinz/master/wave.gif" width="30px" height="30px" /> Hey, I'm Jaynik.

I build stuff that solves problems I actually run into.

### 🌟 Featured Projects

<!-- <a href="https://github.com/Jaynik-S/Playgenix">
  <img align="center" src="https://github-readme-stats-fast.vercel.app/api/pin/?username=Jaynik-S&repo=Playgenix&theme=tokyonight" />
</a> -->

<a href="https://github.com/Jaynik-S/SentimentScribe">
  <img align="center" src="https://github-readme-stats-fast.vercel.app/api/pin/?username=Jaynik-S&repo=SentimentScribe&theme=tokyonight" />
</a>

<a href="https://github.com/Jaynik-S/PianoAnnotations">
  <img align="center" src="https://github-readme-stats-fast.vercel.app/api/pin/?username=Jaynik-S&repo=PianoAnnotations&theme=tokyonight" />
</a>

### 🔥 Projects I’ve Built


| Repository | Description | Language/Technologies |
|---|---|---|
| [**FocusArc**](https://github.com/Jaynik-S/FocusArc) | Personal study-time tracking web app with course timers, session history, and weekly analytics. | TypeScript, React, Python, FastAPI, SQLAlchemy, PostgreSQL, Docker |
| [**AutomatedBookmarking**](https://github.com/Jaynik-S/AutomatedBookmarking) | Chrome extension for X community timelines with timeframe filters, thresholds, and automated bookmarking. | JavaScript, HTML/CSS, Chrome Extension APIs, Chrome Storage API |
| [**NBA_Insights**](https://github.com/Jaynik-S/NBA_Insights) | Data-driven NBA analytics web app with archetype classification, career comparisons, and interactive visualizations. | Python, Flask, Pandas, scikit-learn, Matplotlib, Chart.js |
| [**UofT_Adventure_Game**](https://github.com/Jaynik-S/UofT_Adventure_Game) | UofT-themed adventure game with campus navigation, item quests, and stamina/inventory management. | Python, Flask, JavaScript, HTML/CSS, JSON, Jinja |
| [**Toronto_Crime_Visualization**](https://github.com/Jaynik-S/Toronto_Crime_Visualization) | Toronto crime analysis CLI with hierarchy trees, Graphviz visuals, and ranking/search tools. | Python, CSV, Graphviz |

### 🛠️ Open-Source Forks & Experiments

| Repository | Description | Language/Technologies |
|---|---|---|
| [**videopianonotes**](https://github.com/Jaynik-S/video2pianonotes) | Video-to-piano-annotation pipeline with YouTube/local input, MIDI extraction, and JSON/ASCII/HTML annotations. | Python, Bash, OpenCV, Pygame, MIDIUtil, pretty_midi, yt-dlp |
| [**block-site**](https://github.com/Jaynik-S/block-site) | Site-blocking extension upgrade with daily limits, live countdowns, and +5-minute extensions. | TypeScript, Chrome Extension APIs, Jest |


### 🔗 Connect

- [Portfolio](https://jaynik.codes)
- [LinkedIn](https://www.linkedin.com/in/jayniksheth/)
- [Email](mailto:jayniksheth3@gmail.com)
